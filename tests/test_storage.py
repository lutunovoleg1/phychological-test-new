...
def temp_db():
    ...
    if db_path.exists():
        try:
            ...
            # Игнорируем ошибки удаления на Windows, если файл еще занят
            pass
...
class TestResultsStorage:
    ...
    def test_save_result(self, storage):
        ...
        with sqlite3.connect(storage.db_path) as conn:
            cursor = conn.execute(
                "SELECT patient_name, patient_age, patient_education, total_time_seconds, errors_count FROM results"
            )
            row = cursor.fetchone()
    ...
    def test_result_rounding(self, storage):
        ...
        with sqlite3.connect(storage.db_path) as conn:
            cursor = conn.execute("SELECT total_time_seconds FROM results")
    ...
    def test_result_with_zero_errors(self, storage):
        ...
        with sqlite3.connect(storage.db_path) as conn:
            cursor = conn.execute("SELECT errors_count FROM results WHERE patient_name='БезОшибок'")
    ...
    def test_result_with_many_errors(self, storage):
        ...
        with sqlite3.connect(storage.db_path) as conn:
            cursor = conn.execute("SELECT errors_count FROM results WHERE patient_name='МногоОшибок'")
            errors = cursor.fetchone()[0]
    ...
    def test_database_persistence(self, temp_db):
        ...
        with sqlite3.connect(storage2.db_path) as conn:
            cursor = conn.execute("SELECT patient_name FROM results")
            name = cursor.fetchone()[0]

        assert name == "Персистентность"
        storage2.close()
    ...
    def test_date_stored(self, storage):
        ...
        with sqlite3.connect(storage.db_path) as conn:
            cursor = conn.execute("SELECT test_date FROM results WHERE patient_name='ДатаТест'")
            date_str = cursor.fetchone()[0]

        saved_date = datetime.fromisoformat(date_str)
        assert before_save <= saved_date <= after_save
    ...
    def test_edge_age_values(self, storage):
        ...
        with sqlite3.connect(storage.db_path) as conn:
            cursor = conn.execute("SELECT patient_age FROM results ORDER BY id")
